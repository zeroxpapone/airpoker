...
export default function HomePage() {
  const { user, username, isRegisteredUser, photoURL, logout, linkGuestToRegistered } = useAuth();
  ...
  // Invitations listener in HomePage
  useEffect(() => {
    if (!user) return;
    const qInvitations = collection(db, "users", user.uid, "invitations");
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      // Sort client-side by endedAt desc
      list.sort((a, b) => {
  ...
  return (
    <div className="dashboard-layout" id="dashboard-page-container">
      ...
      <div 
        className={`glass-panel profile-widget-panel ${isRegisteredUser ? "clickable" : ""}`} 
        id="profile-widget"
        onClick={() => {
          if (isRegisteredUser) {
            navigate("/profile");
        ...
        style={{ cursor: isRegisteredUser ? "pointer" : "default" }}
      >
      ...
      <div className="dashboard-actions-grid">
        ...
        <div 
          className="glass-panel stats-card-panel clickable" 
          id="card-stats-display"
          onClick={() => {
            if (isRegisteredUser) {
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
              ...
              {leaderboard.length === 0 ? (
                ...
                leaderboard.map((item, idx) => (
                  <div 
                    key={item.uid}
                    className={`list-item-row clickable ${item.uid === user?.uid ? "highlighted" : ""}`}
                    onClick={() => {
                ...
                {friends.length === 0 ? (
                  ...
                  friends.map((friend) => (
                    <div 
                      ...
                      className={`list-item-row ${friend.status === "ACCEPTED" ? "clickable" : ""}`}
                      onClick={() => {
      ...
      {showUpgradeModal && (
        <div className="modal-overlay" id="modal-upgrade-account">
          <div className="glass-panel modal-panel-box">
            ...
            <div style={{ display: "flex", gap: "0.5rem", width: "100%" }} id="upgrade-social-group">
              <button
                ...
                return (
                  <div 
                    ...
                    style={{ 
                      ...
                      backgroundColor: isSelf ? "rgba(255, 255, 255, 0.03)" : "transparent"
                    }}
                  >
          ...
          <div className="glass-panel modal-panel-box" style={{ maxWidth: "340px", padding: "1.5rem", textAlign: "center", display: "flex", flexDirection: "column", gap: "1rem" }}>
            ...
            <div>
              <h3 style={{ fontSize: "1.2rem", fontWeight: 800, fontFamily: "var(--font-display)" }}>
                {t("dashboard.invitePopupTitle") || "Nuovo Invito al Tavolo!"}
              </h3>
              <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", marginTop: "0.5rem", lineHeight: 1.4 }}>
                {t("dashboard.invitePopupDesc", { sender: invitations[0].senderUsername, table: invitations[0].tableName }) ||
                  `@${invitations[0].senderUsername} ti ha invitato a giocare al tavolo: ${invitations[0].tableName}`}
              </p>
            </div>
            ...
            <div style={{ display: "flex", gap: "0.6rem", marginTop: "0.5rem" }}>
              <button
                ...
                onClick={() => handleDeclineInvitation(invitations[0])}
                className="poker-btn-secondary"
                style={{ flex: 1, padding: "0.65rem", borderRadius: "99px", fontWeight: 700 }}
              >
                {t("dashboard.btnDecline") || "Rifiuta"}
              </button>
              <button
                id="btn-accept-invitation"
                onClick={() => handleAcceptInvitation(invitations[0])}
                className="poker-btn-primary"
                style={{ flex: 1, padding: "0.65rem", borderRadius: "99px", fontWeight: 800, color: "var(--text-inverse)" }}
              >
                {t("dashboard.btnAccept") || "Accetta"}
              </button>
            </div>
          </div>
        </div>
